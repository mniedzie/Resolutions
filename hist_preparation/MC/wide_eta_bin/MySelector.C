...
{
//	instead of reading PU weight from .root file i print it once out and save it an array. Opening it for each event takes too long. However the code below (the commented out one) does it by reading it from a .root file.

// Here I calculate weight basing on pileup and ptbin. 
...
void MySelector::SlaveBegin(TTree * /*tree*/)
{
   ...
   // The tree argument is deprecated (on PROOF 0 is passed).
	...
	EtaBinsNo = 6;				// number of bins for standard calculation
	EtaFtBinsNo = 2;			// number of bins for forward extension (with forward pt binning)
	EtaFtControlBinsNo = 4;	// number of bins for forward extension in central region (with central pt binning). First bin here is used as reference region.
   ...
   TString option = GetOption();
	...
	//////////////////////////////////////////////////////////////////////////////////////////////////////
	// Below I define histograms. This could be made more efficient by defining functions to do that. 	   
	// Even more efficiently do it by defining class...																   
	//////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	for( int m = 0; m < EtaBinsNo; m++ )
	{
		...
		std::vector< std::vector< TH1F* > > gen_temp2, gen_temp2pt;

		for( int p = 0; p < PtBinsNo; p++ )
		{
			std::vector< TH1F* > temp1, temp1pt;
			std::vector< TH1F* > gen_temp1, gen_temp1pt;
			...
			gen_temp2.push_back(gen_temp1);
			gen_temp2pt.push_back(gen_temp1pt);
		}
		asymmetries_all.push_back(temp2);
		asymmetries_pt_all.push_back(temp2pt);

		asymmetries_gen_all.push_back(gen_temp2);
		asymmetries_gen_pt_all.push_back(gen_temp2pt);
	}
	...
	for( int m = 0; m < EtaFtBinsNo; m++ )
	{
		...
		for( int p = 0; p < PtFTBinsNo; p++ )
		{
			...
			for( int r = 0; r < 5; r++ )
			{
				TString name_fw = "forward_"; name_fw += "probe"; name_fw += m+6; name_fw += "_pt"; name_fw += p+1; name_fw += "_alpha"; name_fw += r+1; 
				TString name_fw_pt = "forwardpt_"; name_fw_pt += "probe"; name_fw_pt += m+6; name_fw_pt += "_pt"; name_fw_pt += p+1; name_fw_pt += "_alpha"; name_fw_pt += r+1; 

				TString gen_name_fw = "gen_forward_"; gen_name_fw += "probe"; gen_name_fw += m+6; gen_name_fw += "_pt"; gen_name_fw += p+1; gen_name_fw += "_alpha"; gen_name_fw += r+1; 
				TString gen_name_fw_pt = "gen_forwardpt_"; gen_name_fw_pt += "probe"; gen_name_fw_pt += m+6; gen_name_fw_pt += "_pt"; gen_name_fw_pt += p+1; gen_name_fw_pt += "_alpha"; gen_name_fw_pt += r+1; 
		...
		forward_gen_hist.push_back(gen_temp2);
		forward_gen_pt_hist.push_back(gen_temp2pt);
	}
	...
	for( int m = 0; m < EtaFtControlBinsNo; m++ )
	{
		...
		forward_hist_dijet.push_back(temp2);
		forward_pt_hist_dijet.push_back(temp2pt);

		forward_gen_hist_dijet.push_back(gen_temp2);
		forward_pt_gen_hist_dijet.push_back(gen_temp2pt);
	}
}
...
void MySelector::Terminate()
{
   ...
   // the results graphically or save the results to file.
	...
	// this part is extremely lame. one should make some function that you just give vector and it writes it to a file.
