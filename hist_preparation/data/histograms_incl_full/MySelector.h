...
public :
   ...
   TTree          *fHLT;   //!pointer to the analyzed TTree or TChain
...
//  TBranch        *b_charge;   //!
   ...
   virtual void    Terminate();
	...
	std::vector< std::vector< std::vector< TH1F* > > > forward_dijet_hist;
	std::vector< std::vector< std::vector< TH1F* > > > asymmetries_all;

	std::vector< std::vector< std::vector< TH1F* > > > forward_pt_hist;
	std::vector< std::vector< std::vector< TH1F* > > > forward_dijet_pt_hist;
...
//   charge = 0;
   ...
   fEvent->SetBranchAddress("npuIT", &npuIT, &b_npuIT);
	...
	fHLT = (TTree*)currentFile->Get("hlt/t");
	fHLT -> SetMakeClass(1);
   fHLT->SetBranchAddress("objects_pt", &objects_pt, &b_objects_pt);
   fHLT->SetBranchAddress("paths", &paths, &b_paths);

//   std::cout << "file" << currentFile << std::endl;
//	fHLT = (TTree*)currentFile->Get("hlt/t");
  // std::cout << "tree" << fHLT << std::endl;
