...
void histLoadAsym( TFile &f, bool data, TString name, std::vector< std::vector< std::vector< TH1F* > > > &Asymmetry , std::vector< std::vector< std::vector< TH1F* > > > &GenAsymmetry , int ptbin );
...
void histLinFitB( std::vector< std::vector< TH1F* > > widths_hist_all , std::vector< std::vector< double > > &Widths, std::vector< std::vector< double > > &WidthsError  );
...
void histWidthFE( bool data, std::vector<std::vector<std::vector<TH1F*> > > &Asymmetry , std::vector<std::vector<std::vector<double> > > &Widths, std::vector<std::vector<std::vector<double> > > &WidthsError, int ptbin  );
...
//	bool data = true;
	...
	int alpha_bins = 5, pt_bins = 9, pt_bins_fe = 6, eta_bins = 6, eta_bins_fe = 2, eta_bins_fec = 4;
	...
	histLoadAsym( *f_data, 	true, "asymm_eta", asymmetries_all_data, asymmetries_gen_all_data, pt_bins );	// load asymm from data
	histLoadAsym( *f, 	data, "asymm_eta", asymmetries_all, asymmetries_gen_all, pt_bins );		// load asymm from mc (all)

	histLoadAsym( *f_data, 	true, "asymmpt_probe", asymmetries_all_data_pt, asymmetries_gen_all_data, pt_bins );	// load asymm pt from data
	histLoadAsym( *f, 	data, "asymmpt_probe", asymmetries_all_pt, asymmetries_gen_all_pt, pt_bins );		// load asymm pt from mc (all)
...
//		End of lin-fit                                                      //
////////////////////////////////////////////////////////////////////////////
//		I fit line again but to get const parameter.                        //
////////////////////////////////////////////////////////////////////////////

	std::vector< std::vector< double > > widthsb_all, widthsb_gen_all, widthsb_all_data;
	std::vector< std::vector< double > > widthsb_all_error, widthsb_gen_all_error, widthsb_all_error_data;

	std::vector< std::vector< double > > widthsb_FE, widthsb_gen_FE, widthsb_FE_data;
	std::vector< std::vector< double > > widthsb_FE_error, widthsb_gen_FE_error, widthsb_FE_error_data;

	histLinFitB( widths_hist_all , widthsb_all, widthsb_all_error );
	histLinFitB( widths_hist_all_gen , widthsb_gen_all, widthsb_gen_all_error );
	histLinFitB( widths_hist_all_data , widthsb_all_data, widthsb_all_error_data );

	histLinFitB( widths_hist_FE , widthsb_FE, widthsb_FE_error );
	histLinFitB( widths_hist_FE_gen , widthsb_gen_FE, widthsb_gen_FE_error );
	histLinFitB( widths_hist_FE_data , widthsb_FE_data, widthsb_FE_error_data );

////////////////////////////////////////////////////////////////////////////
//		End of lin-fit                                                      //
////////////////////////////////////////////////////////////////////////////
//		I make histograms of KFSR in P_T bins                               //
////////////////////////////////////////////////////////////////////////////

	std::vector< TH1F* > KFSR_vs_pt_all, KFSR_vs_pt_gen_all, KFSR_vs_pt_all_data;
	std::vector< TH1F* > KFSR_vs_pt_FE, KFSR_vs_pt_gen_FE, KFSR_vs_pt_FE_data;

	fill_widths_pt( "KFSR_Vs_Pt_", KFSR_vs_pt_all, widths_all, widths_all_error, asymmetries_width_pt );
	fill_widths_pt( "KFSR_Vs_Pt_gen_", KFSR_vs_pt_gen_all, widths_gen_all, widths_gen_all_error, asymmetries_gen_width_pt );
	fill_widths_pt( "KFSR_Vs_Pt_data_", KFSR_vs_pt_all_data, widths_all_data, widths_all_error_data, asymmetries_width_data_pt );
	
	fill_widths_pt( "KFSR_Vs_PtFE_", KFSR_vs_pt_FE, widths_FE, widths_FE_error, forward_width_pt );
	fill_widths_pt( "KFSR_Vs_PtFE_gen_", KFSR_vs_pt_gen_FE, widths_gen_FE, widths_gen_FE_error, forward_gen_width_pt );
	fill_widths_pt( "KFSR_Vs_PtFE_data_", KFSR_vs_pt_FE_data, widths_FE_data, widths_FE_error_data, forward_width_data_pt );

////////////////////////////////////////////////////////////////////////////
//		I made histograms of KFSR in P_T bins                               //
////////////////////////////////////////////////////////////////////////////
//		I make histograms of KFSR ratios (for Mikkos check)                 //
//		and histograms of ratio of widths(alpha=0.15)                       //
////////////////////////////////////////////////////////////////////////////
...
////////////////////////////////////////////////////////////////////////////
//		I finally correct everything for PLI correct way                    //
////////////////////////////////////////////////////////////////////////////
	...
	correctJERwithPLI( JER_MC, JER_MC_error, widths_FSRcorr_all, widths_FSRcorr_all_error, widths_FSRcorr_gen_all, widths_FSRcorr_gen_all_error);
	correctJERwithPLI( JER_data, JER_data_error, widths_FSRcorr_all_data, widths_FSRcorr_all_error_data, widths_FSRcorr_gen_all, widths_FSRcorr_gen_all_error);

	correctJERwithPLI( JER_uncor_FE_MC, JER_uncor_FE_MC_error, widths_FSRcorr_FE, widths_FSRcorr_FE_error, widths_FSRcorr_gen_FE, widths_FSRcorr_gen_FE_error);
	correctJERwithPLI( JER_uncor_FE_data, JER_uncor_FE_data_error, widths_FSRcorr_FE_data, widths_FSRcorr_FE_error_data, widths_FSRcorr_gen_FE, widths_FSRcorr_gen_FE_error);

////////////////////////////////////////////////////////////////////////////
//		I corrected everything for PLI correct way                          //
////////////////////////////////////////////////////////////////////////////
...
////////////////////////////////////////////////////////////////////////////
//		I correct for PLI using b parameter from line fit to sigma_A        //
////////////////////////////////////////////////////////////////////////////

	std::vector< std::vector< double > > JERb_MC, JERb_data;
	std::vector< std::vector< double > > JERb_MC_error, JERb_data_error;

	std::vector< std::vector< double > > JERb_uncor_FE_MC, JERb_uncor_FE_data;
	std::vector< std::vector< double > > JERb_uncor_FE_MC_error, JERb_uncor_FE_data_error;

	correctJERwithPLI( JERb_MC, JERb_MC_error, widthsb_all, widthsb_all_error, widthsb_gen_all, widthsb_gen_all_error);
	correctJERwithPLI( JERb_data, JERb_data_error, widthsb_all_data, widthsb_all_error_data, widthsb_gen_all, widthsb_gen_all_error);

	correctJERwithPLI( JERb_uncor_FE_MC, JERb_uncor_FE_MC_error, widthsb_FE, widthsb_FE_error, widthsb_gen_FE, widthsb_gen_FE_error);

	correctJERwithPLI( JERb_uncor_FE_data, JERb_uncor_FE_data_error, widthsb_FE_data, widthsb_FE_error_data, widthsb_gen_FE, widthsb_gen_FE_error);
...
////////////////////////////////////////////////////////////////////////////
//		I do same thing for widths calculated using b parameter             //
////////////////////////////////////////////////////////////////////////////

	std::vector< std::vector< double > > JERb_FE_MC, JERb_FE_data;
	std::vector< std::vector< double > > JERb_FE_MC_error, JERb_FE_data_error;

	correctForRef( "mccorrectedb", JERb_FE_MC, JERb_FE_MC_error, JERb_uncor_FE_MC, JERb_uncor_FE_MC_error, forward_width_pt  );
	correctForRef( "datacorrectb", JERb_FE_data, JERb_FE_data_error, JERb_uncor_FE_data, JERb_uncor_FE_data_error, forward_width_data_pt  );

////////////////////////////////////////////////////////////////////////////
//		Ref region corrected for method with b parameter                    //
////////////////////////////////////////////////////////////////////////////
...
////////////////////////////////////////////////////////////////////////////
	...
	makeScales( scales_fe_control, scales_fe_control_error, JER_uncor_FE_data, JER_uncor_FE_data_error, JER_uncor_FE_MC, JER_uncor_FE_MC_error );


	for ( int m = 0; m < scales_fe.size(); m++)
	{
		for ( int i = 0; i < scales_fe.at(m).size(); i++ )
		{
...
// and same thing for using-b-parameter calculations
	...
	std::vector< std::vector< double > > scalesb_classic, scalesb_fe, scalesb_fe_control;
	std::vector< std::vector< double > > scalesb_classic_error, scalesb_fe_error, scalesb_fe_control_error;

	makeScales( scalesb_classic, scalesb_classic_error, JERb_data, JERb_data_error, JERb_MC, JERb_MC_error );
	makeScales( scalesb_fe, scalesb_fe_error, JERb_FE_data, JERb_FE_data_error, JERb_FE_MC, JERb_FE_MC_error );

	makeScales( scalesb_fe_control, scalesb_fe_control_error, JERb_uncor_FE_data, JERb_uncor_FE_data_error, JERb_uncor_FE_MC, JERb_uncor_FE_MC_error );
...
// and again, same for b-parameter calculations

	std::vector< TH1F* > JERb_MC_hist, JERb_data_hist, JERb_scale_hist;
	std::vector< TH1F* > JERb_FE_MC_hist, JERb_FE_data_hist, JERb_FE_scale_hist;

	fill_hist( "MC_JERbb_classic", JERb_MC_hist, JERb_MC, JERb_MC_error, asymmetries_width_pt, 0.2);
	fill_hist( "data_JERbb_classtc", JERb_data_hist, JERb_data, JERb_data_error, asymmetries_width_data_pt, 0.2);
	fill_hist( "SFb_classic", JERb_scale_hist, scalesb_classic, scalesb_classic_error, asymmetries_width_data_pt, 2.);

	fill_hist( "MC_JERbb_FE", JERb_FE_MC_hist, JERb_FE_MC, JERb_FE_MC_error, forward_width_pt, 0.2);
	fill_hist( "data_JERbb_FE", JERb_FE_data_hist, JERb_FE_data, JERb_FE_data_error, forward_width_data_pt, 0.2);
	fill_hist( "SFb_FE", JERb_FE_scale_hist, scalesb_fe, scalesb_fe_error, forward_width_data_pt, 2.0);

	std::vector< TH1F* > JERb_FE_MC_hist_control, JERb_FE_data_hist_control, JERb_FE_scale_hist_control;

	fill_hist( "MC_JERbb_FE_control", JERb_FE_MC_hist_control, JERb_uncor_FE_MC, JERb_uncor_FE_MC_error, forward_width_pt, 0.2);
	fill_hist( "data_JERbb_FE_control", JERb_FE_data_hist_control, JERb_uncor_FE_data, JERb_uncor_FE_data_error, forward_width_data_pt, 0.2);
	fill_hist( "SFb_FE_control", JERb_FE_scale_hist_control, scalesb_fe_control, scalesb_fe_control_error, forward_width_data_pt, 2.0);
...
//////////////////////////////////////////////////////////////////////////
//		I subtract reference region width.
//////////////////////////////////////////////////////////////////////////
/*
	std::vector< TH1F* > FE_corrected, FE_gen_corrected, FE_data_corrected;
	std::vector< TH1F* > FE_dijet_corrected, FE_dijet_gen_corrected, FE_dijet_data_corrected;

	correctForRef( "FE_minus_Ref_", FE_corrected, widths_vs_pt_FE, widths_vs_pt_all.at(0));
	correctForRef( "FE_gen_minus_Ref_", FE_gen_corrected, widths_vs_pt_gen_FE, widths_vs_pt_gen_all.at(0));
	correctForRef( "FE_minus_Ref_data_", FE_data_corrected, widths_vs_pt_FE_data, widths_vs_pt_all_data.at(0));

	correctForRef( "FE_dijet_minus_Ref_", FE_dijet_corrected, widths_dijet_vs_pt_FE, widths_vs_pt_all_FT.at(0));
	correctForRef( "FE_dijet_gen_minus_Ref_", FE_dijet_gen_corrected, widths_dijet_vs_pt_gen_FE, widths_vs_pt_gen_all_FT.at(0));
	correctForRef( "FE_dijet_minus_Ref_data_", FE_dijet_data_corrected, widths_dijet_vs_pt_FE_data, widths_vs_pt_all_data_FT.at(0));

//////////////////////////////////////////////////////////////////////////
//		I correct for PLI
//////////////////////////////////////////////////////////////////////////

	std::vector< TH1F* > FE_PLIcorrected, FE_dijet_PLIcorrected;
	std::vector< TH1F* > FE_PLIcorrected_data, FE_dijet_PLIcorrected_data;

	std::vector< TH1F* > widths_PLI_corrected, widths_data_PLI_corrected;

	correctForPLI( "FE_PLIcorrected_", FE_PLIcorrected, FE_corrected, FE_gen_corrected);
	correctForPLI( "FE_PLIcorrected_data_", FE_PLIcorrected_data, FE_data_corrected, FE_gen_corrected);

	correctForPLI( "FE_dijet_PLIcorrected_", FE_dijet_PLIcorrected, FE_dijet_corrected, FE_dijet_gen_corrected);
	correctForPLI( "FE_dijet_PLIcorrected_data_", FE_dijet_PLIcorrected_data, FE_dijet_data_corrected, FE_dijet_gen_corrected);

	correctForPLI( "widths_PLIcorrected_", widths_PLI_corrected, widths_vs_pt_all, widths_vs_pt_gen_all);
	correctForPLI( "widths_PLIcorrected_data_", widths_data_PLI_corrected, widths_vs_pt_all_data, widths_vs_pt_gen_all);*/
...
//////////////////////////////////////////////////////////////////////////
	...
	for( int m = 0; m < forward_width_data_pt.size(); m++ )
	{
		for( int p = 0; p < forward_width_data_pt.at(m).size(); p++ )
		{
//			std::cout << m << ", " << p << ", " << forward_width_data_pt.at(m).at(p).at(4) << std::endl;
	...
	for( int m = 0; m < scales_fe.size(); m++ )
	{
		for( int p = 0; p < scales_fe.at(m).size(); p++ )
		{
//			std::cout << m << ", " << p << ", " << scales_fe.at(m).at(p) << std::endl;
	...
	TFile hprim("widths.root","RECREATE");

	for( int m = 0; m < widths_hist_FE.size(); m++ )
	{
		for( int p = 0; p < widths_hist_FE.at(m).size(); p++ )
		{
	...
	for( int m = 0; m < widths_hist_all.size(); m++ )
	{
		for( int p = 0; p < widths_hist_all.at(m).size(); p++ )
		{
	...
	for( int m = 0; m < KFSR_vs_pt_all.size(); m++ )
	{
		KFSR_vs_pt_all.at(m) -> Write();
	}

	for( int m = 0; m < KFSR_vs_pt_gen_all.size(); m++ )
	{
		KFSR_vs_pt_gen_all.at(m) -> Write();
	}

	for( int m = 0; m < KFSR_vs_pt_all_data.size(); m++ )
	{
		KFSR_vs_pt_all_data.at(m) -> Write();
	}

	for( int m = 0; m < KFSR_vs_pt_FE.size(); m++ )
	{
		KFSR_vs_pt_FE.at(m) -> Write();
	}

	for( int m = 0; m < KFSR_vs_pt_gen_FE.size(); m++ )
	{
		KFSR_vs_pt_gen_FE.at(m) -> Write();
	}

	for( int m = 0; m < KFSR_vs_pt_FE_data.size(); m++ )
	{
	...
	TFile gprim("JERs.root","RECREATE");

	for( int m = 0; m < JER_MC_hist.size(); m++ )
	{
		JER_MC_hist.at(m) -> Write();
	}

	for( int m = 0; m < JER_data_hist.size(); m++ )
	{
		JER_data_hist.at(m) -> Write();
	}

	for( int m = 0; m < JER_FE_MC_hist.size(); m++ )
	{
		JER_FE_MC_hist.at(m) -> Write();
	}

	for( int m = 0; m < JER_FE_data_hist.size(); m++ )
	{
		JER_FE_data_hist.at(m) -> Write();
	}

	for( int m = 0; m < JER_FE_MC_hist_control.size(); m++ )
	{
		JER_FE_MC_hist_control.at(m) -> Write();
	}
	for( int m = 0; m < JER_FE_data_hist_control.size(); m++ )
	{
	...
	TFile gprimb("JERbs.root","RECREATE");

	for( int m = 0; m < JERb_MC_hist.size(); m++ )
	{
		JERb_MC_hist.at(m) -> Write();
	}

	for( int m = 0; m < JERb_data_hist.size(); m++ )
	{
		JERb_data_hist.at(m) -> Write();
	}

	for( int m = 0; m < JERb_FE_MC_hist.size(); m++ )
	{
		JERb_FE_MC_hist.at(m) -> Write();
	}

	for( int m = 0; m < JERb_FE_data_hist.size(); m++ )
	{
		JERb_FE_data_hist.at(m) -> Write();
	}

	for( int m = 0; m < JERb_FE_MC_hist_control.size(); m++ )
	{
		JERb_FE_MC_hist_control.at(m) -> Write();
	}
	for( int m = 0; m < JERb_FE_data_hist_control.size(); m++ )
	{
		JERb_FE_data_hist_control.at(m) -> Write();
	}

	gprimb.Close();


	TFile gbis("SFs.root","RECREATE");

	for( int m = 0; m < JER_scale_hist.size(); m++ )
	{
		JER_scale_hist.at(m) -> Fit("pol0","");
		JER_scale_hist.at(m) -> Write();
	}

	for( int m = 0; m < JER_FE_scale_hist.size(); m++ )
	{
		JER_FE_scale_hist.at(m) -> Fit("pol0","");
		JER_FE_scale_hist.at(m) -> Write();
	}

	for( int m = 0; m < JER_FE_scale_hist_control.size(); m++ )
	{
		JER_FE_scale_hist_control.at(m) -> Fit("pol0","");
		JER_FE_scale_hist_control.at(m) -> Write();
	}

	for( int m = 0; m < JER015_scale_hist.size(); m++ )
	{
		JER015_scale_hist.at(m) -> Fit("pol0","");
		JER015_scale_hist.at(m) -> Write();
	}

	for( int m = 0; m < JER015_FE_scale_hist.size(); m++ )
	{
		JER015_FE_scale_hist.at(m) -> Fit("pol0","");
		JER015_FE_scale_hist.at(m) -> Write();
	}

	double eta_binning [] = { 0., 0.8, 1.3, 1.9, 2.5, 3.0, 3.2, 4.7 };

	TH1F* SFvsEta = new TH1F("Scale factors","Standard" ,7, eta_binning ); 
	SFvsEta ->GetYaxis()->SetTitle("SF");	SFvsEta ->GetXaxis()->SetTitle("#eta");	SFvsEta -> Sumw2();

	double sf_value[] = { 1.131, 1.072, 1.138, 1.116, 1.282 };
	double sf_error[] = { 0.004, 0.007, 0.008, 0.015, 0.046 };

	for (int i = 0; i < 5; i++)
	{
		SFvsEta -> SetBinContent( i + 1 , sf_value[i] );
		SFvsEta -> SetBinError( i + 1 , sf_error[i] );
	}

	SFvsEta ->GetYaxis()-> SetRangeUser( 0., 2. );

	SFvsEta -> Write();

	TH1F* SFvsEta2 = new TH1F("Scale factors2","Forward ext" ,7, eta_binning ); 
	SFvsEta2 ->GetYaxis()->SetTitle("SF");	SFvsEta2 ->GetXaxis()->SetTitle("#eta");	SFvsEta2 -> Sumw2();

	double sf_value2[] = { 1.166, 1.082, 1.359, 1.19, 1.324 };
	double sf_error2[] = { 0.015, 0.023, 0.028, 0.03, 0.055 };

	for (int i = 0; i < 5; i++)
	{
		SFvsEta2 -> SetBinContent( i + 3 , sf_value2[i] );
		SFvsEta2 -> SetBinError( i + 3 , sf_error2[i] );
	}

	SFvsEta2 ->GetYaxis()-> SetRangeUser( 0., 2. );

	SFvsEta2 -> Write();

	TH1F* SFvsEta3 = new TH1F("Scale factors3","b-standard" ,7, eta_binning ); 
	SFvsEta3 ->GetYaxis()->SetTitle("SF");	SFvsEta3 ->GetXaxis()->SetTitle("#eta");	SFvsEta3 -> Sumw2();

	double sf_value3[] = { 1.061, 1.088, 1.092, 1.126, 1.494 };
	double sf_error3[] = { 0.015, 0.027, 0.031, 0.063, 0.198 };

	for (int i = 0; i < 5; i++)
	{
		SFvsEta3 -> SetBinContent( i + 1 , sf_value3[i] );
		SFvsEta3 -> SetBinError( i + 1 , sf_error3[i] );
	}

	SFvsEta3 ->GetYaxis()-> SetRangeUser( 0., 2. );

	SFvsEta3 -> Write();

	TH1F* SFvsEta4 = new TH1F("Scale factors4","b-forward extension" ,7, eta_binning ); 
	SFvsEta4 ->GetYaxis()->SetTitle("SF");	SFvsEta4 ->GetXaxis()->SetTitle("#eta");	SFvsEta4 -> Sumw2();

	double sf_value4[] = { 1.139, 1.062, 1.308, 1.211, 1.32 };
	double sf_error4[] = { 0.048, 0.07, 0.095, 0.062, 0.11 };

	for (int i = 0; i < 5; i++)
	{
		SFvsEta4 -> SetBinContent( i + 3 , sf_value4[i] );
		SFvsEta4 -> SetBinError( i + 3 , sf_error4[i] );
	}

	SFvsEta4 ->GetYaxis()-> SetRangeUser( 0., 2. );
...
//	std::cout << "now i'm fitting scale factors for b-par method" << std::endl;
	...
	for( int m = 0; m < JERb_scale_hist.size(); m++ )
	{
		JERb_scale_hist.at(m) -> Fit("pol0","");
		JERb_scale_hist.at(m) -> Write();
	}

	for( int m = 0; m < JERb_FE_scale_hist.size(); m++ )
	{
		JERb_FE_scale_hist.at(m) -> Fit("pol0","");
		JERb_FE_scale_hist.at(m) -> Write();
	}

	for( int m = 0; m < JERb_FE_scale_hist_control.size(); m++ )
	{
...
*/
	...
	TFile h_merged("widths_merged.root","RECREATE");
